...
function test_lora_adapter_management()
    ...
    try
        ...
        # Test clear adapter (returns nothing if no adapter is set)
...
function test_lora_training_workflow()
    ...
    if !haskey(ENV, "HF_TOKEN")
        @test_skip "Skipping LoRA training test - requires HF_TOKEN environment variable"
        println("⏭️  Skipping LoRA training test (requires HF_TOKEN)")
        println("   Note: Actual training requires internet, Python dependencies, and HF token")
        return
    end

    # result = train_lora_to_gguf("google/gemma-2-2b-it", ENV["HF_TOKEN"], verbose=false)
    # But this still requires internet, Python deps, and takes significant time
